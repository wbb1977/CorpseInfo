...
function CorpseInfo_UpdateTip()
  if GameTooltip:NumLines() == 1 then
    ...
    CorpseInfo_UpdateCTPlayerNotes("friendNotes");
    CorpseInfo_UpdateCTPlayerNotes("ignoreNotes");
    CorpseInfo_UpdateCTPlayerNotes("guildNotes");
...
function CorpseInfo_UpdateCTPlayerNotes(noteType)
  if CT_CoreOptions then
	local charKey = "CHAR-" .. UnitName("player") .. "-" ..GetCVar("realmName");
	if CT_CoreOptions[charKey] and CT_CoreOptions[charKey][noteType] and CT_CoreOptions[charKey][noteType][name] then
      CorpseInfo_AddNote(CT_CoreOptions[charKey][noteType][name]);
	end
  end
end
...
-- 10 June, 2007 - disabled opium support, because current opium version 2.7b does not work with patch 2.1
--
